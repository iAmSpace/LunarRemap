package obf;

import net.minecraft.util.ResourceLocation;

/*
 * Decompiled with CFR 0.150.
 */
public class class_1883 {
    private final ResourceLocation lllIIIllIIIIlllIlIIllIIll;
    private final boolean lllIlIIlIIIlIlIIIllIlllIl;
    private double IlIllllllIIlIIllllIIlIIIl;
    private double lIlllIlllIIIIlIIlllIllIII;

    public class_1883(ResourceLocation class_17732, double d, double d2, boolean bl) {
        this.lllIIIllIIIIlllIlIIllIIll = class_17732;
        this.IlIllllllIIlIIllllIIlIIIl = d;
        this.lIlllIlllIIIIlIIlllIllIII = d2;
        this.lllIlIIlIIIlIlIIIllIlllIl = bl;
    }

    public class_1883(class_1883 class_18832) {
        this.lllIIIllIIIIlllIlIIllIIll = class_18832.lllIIIllIIIIlllIlIIllIIll;
        this.IlIllllllIIlIIllllIIlIIIl = class_18832.IlIllllllIIlIIllllIIlIIIl;
        this.lIlllIlllIIIIlIIlllIllIII = class_18832.lIlllIlllIIIIlIIlllIllIII;
        this.lllIlIIlIIIlIlIIIllIlllIl = class_18832.lllIlIIlIIIlIlIIIllIlllIl;
    }

    public ResourceLocation lllIIIllIIIIlllIlIIllIIll() {
        return this.lllIIIllIIIIlllIlIIllIIll;
    }

    public double lllIlIIlIIIlIlIIIllIlllIl() {
        return this.IlIllllllIIlIIllllIIlIIIl;
    }

    public void lllIIIllIIIIlllIlIIllIIll(double d) {
        this.IlIllllllIIlIIllllIIlIIIl = d;
    }

    public double IlIllllllIIlIIllllIIlIIIl() {
        return this.lIlllIlllIIIIlIIlllIllIII;
    }

    public void lllIlIIlIIIlIlIIIllIlllIl(double d) {
        this.lIlllIlllIIIIlIIlllIllIII = d;
    }

    public boolean lIlllIlllIIIIlIIlllIllIII() {
        return this.lllIlIIlIIIlIlIIIllIlllIl;
    }
}

